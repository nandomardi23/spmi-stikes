...
export default function Edit({ dokumen }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT',
        judul: dokumen.judul, deskripsi: dokumen.deskripsi || '', kategori: dokumen.kategori,
        nomor_dokumen: dokumen.nomor_dokumen || '',
        tanggal_dokumen: dokumen.tanggal_dokumen || '', file: null, is_public: dokumen.is_public,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        post(`/dashboard/dokumen/${dokumen.id}`, {
            forceFormData: true,
        });
    };

    return (
        <DashboardLayout title="Edit Dokumen">
            ...
            <div className="max-w-2xl">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="bg-blue-50 text-blue-800 rounded-xl p-4 text-sm mb-4">
                            Saat ini file dokumen adalah: <a href={`/dashboard/dokumen/${dokumen.id}/download`} className="font-semibold underline">{dokumen.file_path.split('/').pop()}</a>. Upload file baru di bawah ini hanya jika Anda ingin menggantinya.
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1.5">Ganti File Dokumen (Opsional)</label>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <label htmlFor="is_public" className="text-sm text-gray-700">Publikasikan Dokumen</label>
                        </div>
                        <div className="flex gap-3 pt-4">
                            <button type="submit" disabled={processing} className="px-6 py-2.5 bg-linear-to-br from-primary-600 to-primary-700 text-white font-semibold rounded-xl hover:from-primary-700 hover:to-primary-800 disabled:opacity-50 transition shadow-lg shadow-primary-500/25">
                                {processing ? 'Memperbarui...' : 'Perbarui Dokumen'}
                            </button>

...
export default function Create({ roles, unitKerja }) {
    const { data, setData, post, processing, errors } = useForm({ name: '', email: '', password: '', password_confirmation: '', roles: [], unit_kerja_id: '' });
    const handleSubmit = (e) => { e.preventDefault(); post('/dashboard/users'); };
    ...
    return (
        <DashboardLayout title="Tambah Pengguna"><Head title="Tambah Pengguna" />
            <div className="max-w-2xl">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="flex gap-3 pt-4"><button type="submit" disabled={processing} className="px-6 py-2.5 bg-linear-to-br from-primary-600 to-primary-700 text-white font-semibold rounded-xl">Simpan</button><Link href="/dashboard/users" className="px-6 py-2.5 bg-gray-100 text-gray-700 rounded-xl">Batal</Link></div>
                    </form>
                </div>
            </div>
        </DashboardLayout>

...
export default function Edit({ unitKerja, users }) {
    const { data, setData, put, processing, errors } = useForm({ nama: unitKerja.nama, kepala_unit_id: unitKerja.kepala_unit_id || '', kategori: unitKerja.kategori, deskripsi: unitKerja.deskripsi || '' });
    const handleSubmit = (e) => { e.preventDefault(); put(`/dashboard/unit-kerja/${unitKerja.id}`); };

    return (
        <DashboardLayout title="Edit Unit Kerja">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div>
                            ...
                            <input type="text" value={data.nama} onChange={e => setData('nama', e.target.value)} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none" />
                        ...
                        <div className="flex gap-3 pt-4">
                            <button type="submit" disabled={processing} className="px-6 py-2.5 bg-linear-to-br from-primary-600 to-primary-700 text-white font-semibold rounded-xl disabled:opacity-50">Perbarui</button>

...
export default function Edit({ temuan, audits, standarMutu }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        standar_mutu_id: temuan.standar_mutu_id || '', 
        jenis: temuan.jenis || 'observasi', 
        deskripsi: temuan.deskripsi || '', 
        rekomendasi: temuan.rekomendasi || '', 
        batas_waktu: temuan.batas_waktu || '',
        status: temuan.status || 'open'
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(`/dashboard/temuan/${temuan.id}`, {
            onSuccess: () => {
    ...
    return (
        <DashboardLayout title="Edit Temuan Audit">
            <Head title="Edit Temuan Audit" />

            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight">Edit Temuan #{temuan.id}</h2>
                    <p className="text-sm text-gray-500 mt-1">Perbarui detail ketidaksesuaian atau status temuan</p>
                </div>
                <Link 
                    href={temuan.audit_id ? `/dashboard/audit/${temuan.audit_id}` : '/dashboard/temuan'} 
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-bold text-gray-700 mb-2">Batas Waktu Perbaikan</label>
                            <input 
                                type="date" 
                        ...
                        <div>
                            <label className="block text-sm font-bold text-gray-700 mb-2">Status Penyelesaian <span className="text-danger-500">*</span></label>
                            <select 
                                value={data.status} 
                            ...
                            >
                                ...
                                <option value="in_progress">In Progress (Sedang Perbaikan)</option>
                                <option value="verified">Verified (Telah Diverifikasi)</option>
                                <option value="closed">Closed (Selesai)</option>
                            </select>
                            {errors.status && <p className="mt-1.5 text-xs font-bold text-danger-500">{errors.status}</p>}
                        </div>
                    </div>
                    ...
                    <div className="pt-6 border-t border-gray-100 flex justify-end">
                        ...
                        >
                            {processing ? 'Menyimpan...' : 'Perbarui Temuan'}
                        </button>
                    </div>
                </form>
            </div>

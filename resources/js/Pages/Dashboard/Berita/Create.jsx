...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({ judul: '', ringkasan: '', konten: '', gambar: null, is_published: true });
    const handleSubmit = (e) => { e.preventDefault(); post('/dashboard/berita', { forceFormData: true }); };

    return (
        <DashboardLayout title="Tulis Berita"><Head title="Tulis Berita" />
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div>
                            ...
                            <textarea rows={10} value={data.konten} onChange={e => setData('konten', e.target.value)} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none font-mono" placeholder="<p>Isi berita...</p>" />
                        ...
                        <div className="flex gap-3 pt-4">
                            <button type="submit" disabled={processing} className="px-6 py-2.5 bg-linear-to-br from-primary-600 to-primary-700 text-white font-semibold rounded-xl disabled:opacity-50">Simpan Berita</button>

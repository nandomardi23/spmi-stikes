            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {temuans.data.length > 0 ? temuans.data.map(t => (
                            <tr key={t.id} className="hover:bg-gray-50 transition">
                                ...
                                <td className="px-6 py-4 text-gray-500 text-xs">{t.batas_waktu || '-'}</td>
                ...
                {temuans.links && temuans.links.length > 3 && (
                    <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-1">
                        {temuans.links.map((link, i) => <Link key={i} href={link.url || '#'} className={`px-3 py-1.5 text-sm rounded-lg ${link.active ? 'bg-primary-600 text-white' : link.url ? 'text-gray-600 hover:bg-gray-100' : 'text-gray-300'}`} dangerouslySetInnerHTML={{ __html: link.label }} />)}
                    </div>
                )}
            </div>
        </DashboardLayout>
